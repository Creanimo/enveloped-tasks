...
let isTesting = false;
const opts = misc_1.getConfigFile();
opts._cli = true;
for (let i = process.argv.length - 1; i >= 2; --i) {
    ...
    if (arg.indexOf('--port=') > -1) {
        ...
        if (!match)
            misc_1.error('Option --mount is wrong');
        else {
        ...
        console.log(const_1.NAME, const_1.VERSION);
        process.exit();
    }
        ...
        const match = arg.substring(8).match(/([^:]+):(.+)$/);
        if (!match)
            misc_1.error('Option --proxy is wrong');
        else {
...
// Patch paths
opts.root = process.argv[2] ? misc_1.removeLeadingSlash(process.argv[2]) : opts.root;
if (opts.watch) {
    if (opts.length === 1 && opts.watch[0] === 'true')
        ...
        opts.watch = opts.watch.map(function (relativePath) {
            return path_1.default.join(path_1.default.resolve(), relativePath);
        });
    }
}
if (opts.ignore) {
